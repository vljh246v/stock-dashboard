...
export interface DecisionReason {
  category: "trend" | "valuation" | "risk" | "fundamental" | "etf_exposure" | "data_quality";
  label: string;
  detail: string;
  evidence?: string;
}
...
function detectAssetType(profile: any, chart: any): AssetType {
  if (profile?.fundProfile || chart?.meta?.instrumentType === "ETF") return "etf";
  if (profile?.summaryProfile || chart?.meta?.instrumentType === "EQUITY") return "stock";
  return "unknown";
}
...
function buildPriceZones(insights: any, chart: any): DecisionSummary["priceZones"] {
  ...
  const currentPrice = toNumber(chart?.meta?.regularMarketPrice);
  ...
  if (currentPrice) {
    return {
      interestBelow: Number((currentPrice * 0.95).toFixed(2)),
      invalidationBelow: Number((currentPrice * 0.9).toFixed(2)),
      riskManagementNear: Number((currentPrice * 1.1).toFixed(2)),
...
function scoreValuation(insights: any) {
  ...
  if (description.includes("undervalued")) {
    return { score: 1, evidence: `밸류에이션: ${translateFinancialTerm(valuation.description)}` };
  }
  if (description.includes("overvalued")) {
    return { score: -1, evidence: `밸류에이션: ${translateFinancialTerm(valuation.description)}` };
  }
  return { score: 0, evidence: valuation.description ? `밸류에이션: ${translateFinancialTerm(valuation.description)}` : "밸류에이션 데이터 부족" };
}
...
function scoreRecommendation(insights: any) {
  ...
  if (rating.includes("buy") || rating.includes("outperform")) {
    return { score: 1, evidence: `애널리스트 의견: ${translateFinancialTerm(recommendation.rating)}` };
  }
  if (rating.includes("sell") || rating.includes("underperform")) {
    return { score: -1, evidence: `애널리스트 의견: ${translateFinancialTerm(recommendation.rating)}` };
  }
  return { score: 0, evidence: recommendation.rating ? `애널리스트 의견: ${translateFinancialTerm(recommendation.rating)}` : "애널리스트 의견 부족" };
}
...
function scoreRisk(insights: any) {
  ...
  return {
    score,
    evidence: signals.length > 0 ? signals.join(", ") : "특별한 리스크 신호 부족",
  };
}
...
function buildStockSummary(symbol: string, insights: any, chart: any, sources: DecisionSummary["sources"]): DecisionSummary {
  if (!insights) {
    return {
      symbol,
      assetType: "stock",
      ...
      reasons: clampReasons([
        {
          category: "data_quality",
          label: "인사이트 데이터 없음",
          detail: "추세, 밸류에이션, 애널리스트 의견을 충분히 확인하지 못했습니다.",
  ...
  const totalScore = trend.score + valuation.score + recommendation.score + risk.score;
  const state: DecisionState = totalScore >= 1.2 ? "interest" : totalScore <= -0.8 ? "caution" : "wait";
  const confidence: DecisionSummary["confidence"] = Math.abs(totalScore) >= 2 ? "높음" : Math.abs(totalScore) >= 1 ? "중간" : "낮음";

  const reasons: DecisionReason[] = [
    {
      category: "trend",
      label: trend.score > 0 ? "추세 우호적" : trend.score < 0 ? "추세 부담" : "추세 중립",
      detail: trend.score > 0 ? "기술적 흐름은 단기 판단에 긍정적입니다." : trend.score < 0 ? "기술적 흐름은 아직 회복 신호가 약합니다." : "기술적 방향성이 뚜렷하지 않습니다.",
      evidence: trend.evidence,
    },
    {
      category: "valuation",
      label: valuation.score > 0 ? "가격 부담 완화" : valuation.score < 0 ? "밸류에이션 부담" : "가치 판단 보류",
      detail: valuation.score > 0 ? "현재 평가는 상대적으로 부담이 낮은 편입니다." : valuation.score < 0 ? "좋은 기업이어도 진입 가격은 신중히 봐야 합니다." : "가치 평가 데이터가 충분하지 않습니다.",
      evidence: valuation.evidence,
    },
    {
      category: "risk",
      label: risk.score < 0 ? "리스크 확인 필요" : "리스크 신호 제한적",
      detail: risk.score < 0 ? "긍정 요인이 있어도 하방 기준을 함께 확인할 필요가 있습니다." : "뚜렷한 위험 신호는 제한적이지만 변동성은 계속 확인해야 합니다.",
...
function buildEtfSummary(symbol: string, profile: any, chart: any, etfHoldings: any, sources: DecisionSummary["sources"]): DecisionSummary {
  const holdings = Array.isArray(etfHoldings?.holdings) ? etfHoldings.holdings : [];
  const topWeight = holdings.reduce((sum: number, holding: any) => sum + (toNumber(holding.weight) || 0), 0);
  ...
  const expenseRatio = formatPercent(expenseRaw, formatValue(fees.annualReportExpenseRatio));
  const turnoverRaw = toNumber(fees.annualHoldingsTurnover);
  const turnover = formatPercent(turnoverRaw, formatValue(fees.annualHoldingsTurnover));
  ...
  const state: DecisionState = missingCoreData ? "unavailable" : holdings.length === 0 || highCost || concentrated ? "wait" : "interest";
  const labelKo = stateLabel(state);
  ...
  return {
    ...
    state,
    labelKo,
    confidence: holdings.length >= 3 && expenseRaw !== undefined ? "중간" : "낮음",
    headline,
    reasons: clampReasons([
      {
        category: "fundamental",
        label: lowCost ? "장기 비용 부담 낮음" : highCost ? "장기 비용 부담 확인" : expenseRatio ? "보수율 확인 필요" : "보수 데이터 부족",
        detail: expenseRatio
      ...
      },
      {
        ...
        label: holdings.length > 0 ? "구성 집중도 확인" : "구성 종목 데이터 부족",
      ...
      {
        category: "risk",
        label: concentrated ? "집중도 주의" : turnover ? "장기 운용 안정성 확인" : "운용 리스크 확인",
        detail: concentrated
        ...
        evidence: assets ? `AUM ${assets}` : holdings.length > 0 ? `top holdings ${topWeight.toFixed(1)}%` : "ETF risk data partial",
      },
    ]),
    riskNote: "ETF도 시장 하락, 보수 누적, 구성 종목 변경, 특정 섹터나 대형주 집중도에 따라 손실이 날 수 있으므로 장기 보유 전 기준일과 비용을 함께 확인해 주세요.",
...
export function generateDecisionSummary(input: GenerateDecisionSummaryInput): DecisionSummary {
  ...
  if (assetType === "etf") {
    sources.push({
      name: "ETF holdings",
      status: input.etfHoldings?.holdings?.length ? "used" : "unavailable",
      asOf: input.etfHoldings?.asOfDate || undefined,
    });
