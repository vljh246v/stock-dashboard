...
export interface AnalysisPack {
  ...
  tabData: Record<"overview" | "technical" | "financial" | "guidance" | "filings" | "etf" | "sentiment", {
    highlights: string[];
  }>;
...
function toNumber(value: unknown): number | undefined {
  ...
  if (value && typeof value === "object") return toNumber((value as { raw?: unknown }).raw);
...
function buildTechnical(insights: any) {
  ...
  const readableOutlooks = outlooks
    ...
    .map(([label, outlook]) => `${label}: ${translateFinancialTerm(outlook.direction)}${outlook.score !== undefined ? ` ${outlook.score}` : ""}`);
...
function buildFilings(secFilings: any) {
  ...
  return filings
    .map((filing: any) => compact([filing.type, filing.title, filing.date]).join(" · "))
    .filter(Boolean)
    .slice(0, 5);
}

export function generateAnalysisPack(input: GenerateAnalysisPackInput): AnalysisPack {
  ...
  const assetType: AssetType = hasFundProfile || chartMeta.instrumentType === "ETF"
    ? "etf"
  ...
  const bullishPoints = Array.isArray(insights?.upsell?.msBullishSummary) ? insights.upsell.msBullishSummary : [];
  const bearishPoints = Array.isArray(insights?.upsell?.msBearishSummary) ? insights.upsell.msBearishSummary : [];
  const fees = fundProfile.feesExpensesInvestment || {};
  const holdings = Array.isArray(input.etfHoldings?.holdings) ? input.etfHoldings.holdings : [];
  const etf = assetType === "etf"
    ? {
        ...
        topHoldingsWeight: holdings.length > 0
          ? Number(holdings.reduce((sum: number, holding: any) => sum + (toNumber(holding.weight) || 0), 0).toFixed(2))
          : undefined,
  ...
  const price = {
    current: toNumber(chartMeta.regularMarketPrice || profile?.price?.regularMarketPrice),
  ...
  const valuationSummary = compact([
    valuation.description ? `밸류에이션: ${translateFinancialTerm(valuation.description)}` : undefined,
    valuation.discount ? `할인율 ${valuation.discount}` : undefined,
    valuation.relativeValue ? `상대가치 ${translateFinancialTerm(valuation.relativeValue)}` : undefined,
    recommendation.rating ? `애널리스트 ${translateFinancialTerm(recommendation.rating)}` : undefined,
  ]).join(" · ") || "밸류에이션 데이터가 제한적입니다.";

  const governanceSignals = compact([
    companySnapshot.innovativeness !== undefined ? `혁신 ${formatScore(companySnapshot.innovativeness)}` : undefined,
    companySnapshot.hiring !== undefined ? `채용 ${formatScore(companySnapshot.hiring)}` : undefined,
    companySnapshot.sustainability !== undefined ? `지속가능성 ${formatScore(companySnapshot.sustainability)}` : undefined,
    companySnapshot.insiderSentiments !== undefined ? `내부자 심리 ${formatScore(companySnapshot.insiderSentiments)}` : undefined,
  ]);

  const latestFilings = buildFilings(secFilings);
  const insiderTransactions = buildInsiderTransactions(holders);
  const displayName = profile?.price?.shortName || profile?.price?.longName || chartMeta.shortName || chartMeta.longName || symbol;
  ...
  const pack: AnalysisPack = {
    ...
    valuation: {
      ...
      rating: recommendation.rating ? translateFinancialTerm(recommendation.rating) : undefined,
      targetPrice: toNumber(recommendation.targetPrice),
      analystCount: toNumber(recommendation.numberOfAnalysts),
    },
    news: {
      events: events
        .map((event: any) => ({ headline: String(event.headline || ""), date: event.date }))
        .filter((event: { headline: string; date?: string }) => event.headline)
        .slice(0, 8),
      bullishPoints: bullishPoints.filter((point: unknown): point is string => typeof point === "string").slice(0, 5),
      bearishPoints: bearishPoints.filter((point: unknown): point is string => typeof point === "string").slice(0, 5),
    },
    governance: {
    ...
    etf,
    tabData: {
      overview: { highlights: compact([displayName, profile?.summaryProfile?.sector, profile?.summaryProfile?.industry, fundProfile.categoryName]) },
      technical: { highlights: compact([technical.summary, price.support ? `지지선 ${price.support}` : undefined, price.resistance ? `저항선 ${price.resistance}` : undefined]) },
      financial: { highlights: assetType === "etf"
        ? compact([etf?.expenseRatio ? `총보수율 ${etf.expenseRatio}` : undefined, etf?.netAssets ? `AUM ${etf.netAssets}` : undefined])
        : compact([valuationSummary, recommendation.targetPrice ? `목표가 ${recommendation.targetPrice}` : undefined]) },
      guidance: { highlights: compact([...bullishPoints.slice(0, 2), ...bearishPoints.slice(0, 2)]).map(translateFinancialText) },
      filings: { highlights: latestFilings },
      etf: { highlights: compact([
        fundProfile.family,
        fundProfile.categoryName,
        etf?.topHoldingsWeight !== undefined ? `상위 ${etf.holdings.length}개 비중 ${etf.topHoldingsWeight.toFixed(1)}%` : undefined,
      ]) },
      sentiment: { highlights: compact([
        ...events.slice(0, 3).map((event: { headline?: string }) => event.headline),
    ...
    sources: [
      ...
      ...(assetType === "etf" ? [source("ETF holdings", input.etfHoldings, input.etfHoldings?.asOfDate)] : []),
    ],
  };
