...
export default function Dashboard() {
  ...
  const selectedSymbol = params?.symbol
    ? normalizeStockSymbol(params.symbol)
    : "";
  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground overflow-hidden md:flex-row">
      {/* Sidebar */}
      <aside className="flex max-h-56 w-full shrink-0 flex-col overflow-hidden border-b border-border bg-sidebar md:h-screen md:max-h-none md:w-64 md:border-b-0 md:border-r">
        {/* Logo */}
        ...
        {/* Watchlist */}
        <ScrollArea className="min-h-0 shrink-0 md:flex-1">
          <div className="flex gap-2 overflow-x-auto p-2 [scrollbar-width:none] md:block md:space-y-0.5 md:overflow-visible [&::-webkit-scrollbar]:hidden">
            {watchlistQuery.isLoading ? (
              <div className="flex w-full items-center justify-center py-3 md:py-8">
                <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
              </div>
            ) : watchlistItems.length === 0 ? (
              <div className="flex w-full items-center justify-center gap-2 rounded-md px-3 py-3 text-center md:block md:py-8">
                <Search className="h-4 w-4 text-muted-foreground/50 md:mx-auto md:mb-2 md:h-8 md:w-8" />
              ...
              watchlistItems.map(item => (
                <div
                  key={item.id}
                  className={`group flex min-w-[10rem] shrink-0 cursor-pointer items-center justify-between rounded-md px-3 py-2 transition-colors md:min-w-0 ${
                ...
                >
                  ...
                  {item.saved ? (
                    <button
                      ...
                      className="rounded p-1 opacity-100 transition-opacity hover:bg-destructive/20 md:opacity-0 md:group-hover:opacity-100"
                    >
        ...
        {user.role === "admin" && (
          <>
            ...
            <div className="hidden shrink-0 space-y-2 p-3 md:block">
                  ...
                  {pendingUsers.map(pendingUser => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          approveUserMutation.mutate({ userId: pendingUser.id })
                        }
                      >
                        승인
                      </Button>
        ...
        {/* User Footer */}
        <Separator className="hidden md:block" />
        <div className="hidden shrink-0 items-center justify-between p-3 md:flex">
          <span className="text-xs text-muted-foreground truncate">
