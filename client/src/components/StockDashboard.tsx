...
export default function StockDashboard({ symbol }: StockDashboardProps) {
  const analysisPackQuery = trpc.stock.analysisPack.useQuery({ symbol }, { retry: 1 });
  ...
  const isETF = analysisPackQuery.data?.pack.asset.assetType === "etf" || !!(profileData?.fundProfile) || chartMeta?.instrumentType === "ETF";
      ...
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-mono font-semibold">{symbol}</span> 데이터를 불러오고 있습니다.
          </p>
        </div>
      </div>
    );
        ...
        <Card className="bg-card border-border max-w-md w-full">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              <span className="font-mono font-semibold text-foreground">{symbol}</span>에 해당하는 종목 정보를 찾지 못했습니다.
              영문 티커를 다시 확인해 주세요.
            </p>
    ...
    <div className="p-6 space-y-6">
        ...
        <div>
          ...
          {profileData?.summaryProfile?.industry && (
            <p className="text-sm text-muted-foreground mt-1">
              {translateIndustry(profileData.summaryProfile.industry)} · {translateSector(profileData.summaryProfile.sector)}
            </p>
          )}
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="bg-secondary border border-border w-full justify-start overflow-x-auto flex-nowrap">
          ...
          <TabsTrigger value="technical" className="text-xs">차트</TabsTrigger>
          {!isETF && <TabsTrigger value="financial" className="text-xs">재무</TabsTrigger>}
          {isETF
            ? <TabsTrigger value="etf" className="text-xs">ETF 정보</TabsTrigger>
            : <TabsTrigger value="guidance" className="text-xs">가이던스</TabsTrigger>
          }
          {!isETF && <TabsTrigger value="filings" className="text-xs">공시</TabsTrigger>}
          <TabsTrigger value="opinion" className="text-xs">의견</TabsTrigger>
          <TabsTrigger value="sentiment" className="text-xs">뉴스</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4">
          <CompanyOverview data={profileData} isLoading={analysisPackQuery.isLoading} chartMeta={chartMeta} />
        </TabsContent>
