...
const formatOutcome = (outcome: Outcome | undefined) => {
  ...
  const pct =
    typeof outcome.returnPct === "number"
      ? `${outcome.returnPct.toFixed(2)}%`
      : "확인 불가";
...
const badgeVariant = (outcome: Outcome | undefined) => {
  ...
  return outcome.alignment === "방향 일치"
    ? ("default" as const)
    : ("outline" as const);
};

const collectionStatus = (
  oneMonth: Outcome | undefined,
  threeMonth: Outcome | undefined
) =>
  [oneMonth, threeMonth].some(
    outcome => !outcome || outcome.status === "pending"
  )
    ? "수집 중"
    : "확인 가능";

const buildDisplayRows = (rows: TrackingRow[]) =>
  rows.map((row, index) => {
    ...
    return {
      ...
      oneMonth,
      threeMonth,
      status: collectionStatus(oneMonth, threeMonth),
      key: `${row.snapshot.opinionCreatedAt}-${index}`,
    };
  });
    ...
    <Card className="bg-card border-border">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-muted-foreground">
          {tracking?.copy?.description ||
            "이전에 남긴 판단과 이후 가격 변화를 함께 보여줍니다."}
        </p>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <>
            <div className="grid gap-3 md:hidden">
              {displayRows.map(({ row, oneMonth, threeMonth, status, key }) => {
                return (
                  <div
                    key={`${key}-mobile`}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm font-medium">
                          {formatDate(row.snapshot.opinionCreatedAt)}
                        </p>
                      </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-medium">
                          {row.snapshot.finalSignal}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          신뢰도 {row.snapshot.finalConfidence}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                    ...
                    <div className="mt-3 grid gap-2">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Badge variant={badgeVariant(oneMonth)}>
                          {formatOutcome(oneMonth)}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-xs text-muted-foreground">
                        ...
                        <Badge variant={badgeVariant(threeMonth)}>
                          {formatOutcome(threeMonth)}
                        </Badge>
                      </div>
                    </div>
                  </div>
            ...
            <div className="hidden overflow-x-auto md:block">
              <Table>
                ...
                <TableBody>
                  {displayRows.map(
                    ({ row, oneMonth, threeMonth, status, key }) => {
                      return (
                        <TableRow key={key}>
